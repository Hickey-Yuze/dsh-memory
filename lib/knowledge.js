...
// 作为可检索资产并入召回。检索策略：关键词（默认）；配置嵌入源且向量就绪时
// 自动升级 hybrid RRF（片段向量由 host 调度经 embedding.ensureIndexed 差量重嵌，
// 以 updatedAt=文件 mtime 判旧——文件一改自动失效重嵌，无需手动失效）。
...
export function createKnowledgeIndex(dataDir, { config, log, embedding }) {
  ...
  async function search(query, { limit = 3, threshold = 0.05, decayHalfLifeDays = 0 } = {}) {
    const docs = allDocs();
    ...
    if (queryVector) {
      ...
      return rankDocsHybrid(query, docs, { threshold, decayHalfLifeDays, limit, vectorRoute: { queryVector, vectors } });
    }
