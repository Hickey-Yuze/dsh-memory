...
// off 档会话对记忆系统隐身（不写 L0、不调度蒸馏）；只写覆盖不影响捕获。
// 监听器绝不抛错；插件注入的合成消息（producer-owned source，isSyntheticSource）不入库。

import { isSyntheticSource } from './recall.js';

export function attachCapture(ctx, { store, pipeline, config, log, getMode }) {
  ...
  return ctx.on('session/event', (session, event) => {
    try {
      ...
      if (modeOf(sid) === 'off') return; // off 档完全隐身：不写 L0、不调度
      ...
      if (event.type === 'user/message') {
        ...
        if (isSyntheticSource(data.source)) return; // 合成消息（含我们的召回注入）不入库
        role = 'user';
        text = textOf(data.content, { stripCodeBlocks: false });
      } else if (event.type === 'assistant/message') {
        const message = (event.data || {}).message || {};
        if (isSyntheticSource(message.source)) return;
        role = 'assistant';
