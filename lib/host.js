...
function apply(ctx, config) {
  ...
  const recall = createRecall({ store, config: configFacade, log, modes, embedding, dedupe });
  ...
  disposers.push(ctx.inject(['webServer'], (httpCtx) => {
    try {
      return registerRoutes(httpCtx, { store, pipeline, config: configFacade, log, dataDir, modes, embedding });
    } catch (e) {
