    ...
    const run = async () => {
      ...
      try {
        ...
        if (!result.skipped) {
          ...
          knowledge.pruneOrphans();
          // 片段向量差量重嵌（后台；未配置/未就绪时 ensureIndexed 内部跳过；
          // updatedAt=文件 mtime，文件一改自动判旧重嵌）
          if (typeof embedding.ready === 'function' && embedding.ready()) {
            const docs = knowledge.embedDocs();
            if (docs.length > 0) {
              embedding.ensureIndexed(docs)
                .then((r) => { if (r.written > 0) log.info(`知识库片段向量: 新嵌 ${r.written} 条（共 ${docs.length} 片段）`); })
                .catch((e) => log.warn(`知识库片段向量重嵌失败: ${e && e.message ? e.message : e}`));
            }
          }
        }
      } catch (e) {
...
function registerProfileSections(ctx, { store, config, modes, log }) {
  ...
  function stableText(agentId) {
    ...
    return defuseTemplateVars(segments.join('\n\n'));
  }
  ...
  const registerForAgent = (agent) => {
    ...
    try {
      contextDisposers.push(agent.ctx.systemPrompt.context({
        ...
        order: 950,
        // 稳定区文本是每轮组装必经路径：任何异常（宿主升级改契约/数据异常）都降级为
        // 空段（renderPrompt 过滤空文本），绝不抛错炸轮——这是"会话崩溃"的最后防线。
        text: () => {
          try {
            return stableText(agent.id);
          } catch (e) {
            log.warn(`画像稳定区组装失败（本轮省略，降级不炸轮）: ${e.message || e}`);
            return '';
          }
        },
      }));
