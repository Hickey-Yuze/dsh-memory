...
/**
 * 合成消息来源判定：v4 会话格式要求 producer-owned source kind（废弃的 'plugin'
 * 泛型被宿主直接拒绝），自家注入一律 kind:'dsh-memory'；'plugin' 与 plugin 字段
 * 兜底旧消息回放场景。
 */
export function isSyntheticSource(source) {
  if (!source || typeof source !== 'object') return false;
  return source.kind === 'dsh-memory' || source.kind === 'plugin' || source.plugin === 'dsh-memory';
}

/** 位置线索：记忆内容出现本地路径或"知识库/位于/记住…路径"等指针型表述。 */
export const LOCATION_CLUE_RE = /(?:[A-Za-z]:[\\/]|~\/|\/(?:Users|home|root|mnt|data|opt|srv)\b)|(?:位于|存放在|保存在|记住[^，。\n]{0,6}路径|文件夹|知识库)/;

export function createRecall({ store, config, log, modes, embedding, dedupe, knowledge }) {
  function modeOf(sid) {
  ...
  function lastUserText(messages) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (message.source && isSyntheticSource(message.source)) continue;
      const parts = [];
  ...
  // ---------- 位置线索 ----------
  // 指针型记忆（"知识库位于 C:\..."）只给位置不给内容。真机教训：模型"记住位置"后
  // 并不会主动去读文件，最后只能编数据。命中线索时在注入块尾追加行动提示，
  // 让模型先用文件工具读原文——位置是线索不是答案。
  const LOCATION_HINT = '- 提示：上面的记忆里有本地目录/知识库的**位置**（指针）。位置≠内容：若与当前请求相关，先用文件工具（读取/搜索）查看该处实际内容再回答，不要凭位置臆测或编造数据。';

  function appendLocationHint(formatted) {
    if (!formatted || formatted.includes(LOCATION_HINT)) return formatted;
    const start = formatted.indexOf('\n') + 1;
    const body = formatted.slice(start, formatted.lastIndexOf('</recalled-memory>'));
    if (!LOCATION_CLUE_RE.test(body)) return formatted;
    return `${formatted.slice(0, formatted.lastIndexOf('</recalled-memory>'))}${LOCATION_HINT}\n</recalled-memory>`;
  }
  ...
  /** 知识库片段块：给路径给片段，并明示"完整上下文读原文"（文件内容层，位置≠内容的根治）。 */
  function formatKnowledge(hits, kcfg) {
    const budget = Math.max(200, Number(kcfg.maxTotalChars) || 1200);
    const lines = [];
    let used = 0;
    for (const { item } of hits) {
      ...
      const cap = Math.min(600, remaining);
      if (text.length > cap) text = `${text.slice(0, cap)}…`;
      lines.push(`- [${item.relPath}] ${text}（完整上下文请用文件工具读取 ${item.absPath}）`);
      used += text.length + item.relPath.length + 24;
    }
    if (lines.length === 0) return '';
    return `<knowledge-index>\n以下命中了知识库文件片段（文件内容层，可直接引用；需要完整上下文时读原文）：\n${lines.join('\n')}\n</knowledge-index>`;
  }
  ...
  async function recallTextNow(sid, trimmed, cfg) {
    try {
      ...
      const kbReady = !!(knowledge && kcfg.enabled && knowledge.chunkCount() > 0);
      if (records.length === 0 && !kbReady) return '';
      ...
      if (selected.length > 0 && dedupe) {
        dedupe.mark(sid, selected.map(({ item }) => `${item.id}:${item.updatedAt}`));
      }
      const formatted = selected.length > 0
        ? appendLocationHint(enforceTotalBudget(formatRecall(selected, cfg.recall), cfg.recall.maxTotalRecallChars))
        : '';

      // 知识库片段（可选层）：L1 零命中也允许命中文件；检索失败绝不影响 L1 注入
      let kbBlock = '';
      if (kbReady) {
        try {
          const kbLimit = Math.max(1, Number(kcfg.maxResults) || 3);
          const kbHits = await knowledge.search(trimmed, {
            limit: kbLimit,
            threshold: Number.isFinite(Number(kcfg.scoreThreshold)) ? Number(kcfg.scoreThreshold) : 0.05,
            decayHalfLifeDays: cfg.recall.decayHalfLifeDays,
          });
          const kbFresh = kbHits.filter(({ item }) => !injected.has(`${item.id}:${item.updatedAt}`)).slice(0, kbLimit);
          if (kbFresh.length > 0) {
            if (dedupe) dedupe.mark(sid, kbFresh.map(({ item }) => `${item.id}:${item.updatedAt}`));
            kbBlock = formatKnowledge(kbFresh, kcfg);
          }
        } catch (e) {
          log.warn(`知识库检索失败（本轮跳过）: ${e && e.message ? e.message : e}`);
        }
      }

      const out = defuseTemplateVars([formatted, kbBlock].filter(Boolean).join('\n\n'));
      if (!out) return '';
      {
        ...
        log.info(`召回注入 ${selected.length} 条 L1${kbBlock ? ' + 知识库片段' : ''}（会话 ${sid.slice(0, 12)}…，档位 ${mode}）`);
      }
      return out;
    } catch (e) {
    ...
    try {
      ...
      return {
        ...
        messages: [...decision.messages, createUserMessage({
          ...
          source: { kind: 'dsh-memory', form: 'snapshot', sections: [{ name: 'dsh-memory', text }] },
        })],
      };
    } catch (e) {
