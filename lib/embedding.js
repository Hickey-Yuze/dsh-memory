...
export function createEmbeddingService(dataDir, { config, log }) {
  ...
  return {
    ...
    /** 前缀孤儿清理：删除以 prefix 开头但不在 keepIds 中的向量（知识库片段收缩/删除联动）。 */
    prunePrefix(prefix, keepIds) {
      let dropped = 0;
      for (const id of [...vectors.keys()]) {
        if (typeof id === 'string' && id.startsWith(prefix) && !(keepIds && keepIds.has(id))) {
          vectors.delete(id);
          dropped++;
        }
      }
      if (dropped > 0) {
        persist();
        log.info(`向量孤儿清理: 移除 ${dropped} 条（前缀 ${prefix}）`);
      }
      return dropped;
    },
    dispose() {
