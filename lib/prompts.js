...
/**
 * 把文本中的 {{...}} 模板组全角化为 ｛｛…｝｝。
 * DSH 的 prompt 插值器（renderPrompt）对 section/context 文本里的完整 {{name}} 组
 * 做严格校验：名字不合法 → malformed 抛错；合法但未注册 → unknown 抛错——任一都会
 * 炸掉整轮运行且无转义语法。蒸馏产物（Automa/词库模板等内容）天然可能带此类文本，
 * 所有进入稳定区/召回/入库的文本统一过这里。
 */
export function defuseTemplateVars(text) {
...
export function parseL1Output(text, { maxMemories = 8 } = {}) {
  ...
  for (const item of obj.memories) {
    ...
    if (!content) continue;
    out.push({
      content: defuseTemplateVars(content.slice(0, 500)),
...
export function parseL2Output(text) {
  ...
  for (const item of obj.scenes) {
    ...
    scenes.push({
      ...
      title: defuseTemplateVars(title),
      content: defuseTemplateVars(content),
      record_ids: ids,
    });
  }
...
export function parseL3Output(text) {
  ...
  return defuseTemplateVars(persona.slice(0, 4000));
}
