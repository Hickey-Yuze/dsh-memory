    ...
    function KnowledgeCard({ cfg, setCfg }) {
      ...
      return h("div", { style: card },
        h("div", { style: { ...row, justifyContent: "space-between", marginBottom: "8px" } },
          ...
          status ? tag(status.enabled ? (status.files > 0 ? C.green : C.amber) : C.muted, status.enabled ? (status.files > 0 ? `就绪 ${status.files} 文件 / ${status.chunks} 片段` : "未索引到文件") : "关闭") : null,
        ),
        h("div", { style: { fontSize: "12px", color: C.muted, marginBottom: "8px" } },
          "扫描本地知识库目录（如 Obsidian 库），把 md/txt 内容按段落分块并入召回——模型直接拿到文件内容，而不是只知道位置。自动跳过 .obsidian/.git 等噪音目录；保存后每 10 分钟增量扫描。"),
